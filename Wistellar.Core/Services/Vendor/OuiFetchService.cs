...
{
    public partial class OuiFetchService : IOuiSourceService
    {
        private readonly string ouiFileSource;
        private readonly ILogger logger;

        public OuiFetchService(ILoggerFactory loggerFactory, string ouiFileSource)
        {
            this.ouiFileSource = ouiFileSource;
            this.logger = loggerFactory.CreateLogger<OuiFetchService>();
        }

        /// <summary>
        /// Falls back to the public IEEE registry when no local source is configured.
        /// </summary>
        public OuiFetchService(ILoggerFactory loggerFactory) : this(loggerFactory, "https://standards-oui.ieee.org/oui/oui.txt")
        {
        }
        ...
        private static async IAsyncEnumerable<OuiVendorInfo> ParseStream(Stream ouiStream)
        {
            OuiVendorInfo? currentRecord = null;
            ...
            while ((line = await reader.ReadLineAsync()) != null)
            {
                ...
                if (macMatch.Success)
                {
                    ...
                    currentRecord = new OuiVendorInfo
                    {
                ...
                else if (addressMatch.Success && currentRecord != null)
                {
                    ...
                    var address = addressMatch.Groups[1].Value.Trim();
                    if (currentRecord.Address == null)
                        currentRecord.Address = address;
                    else if (currentRecord.City == null)
                        currentRecord.City = address;
                    else if (currentRecord.Country == null)
                        currentRecord.Country = address;
                    else
                        throw new FormatException("Unexpected OUI address line");
                }
            }
